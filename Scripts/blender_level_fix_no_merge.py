...
def putMeshesInSectors():
	...
	for sectorName, sector in dict_staticSectors.items():
		...
		for obj in sector.children:
			if obj.type == 'MESH':
				materialName = obj.data.materials[0].name
				if materialName not in dict_meshes:
					dict_meshes[materialName] = []

				dict_meshes[materialName].append(obj)

		totalObjectsToProcess = 0
		for key, meshArray in dict_meshes.items():
			totalObjectsToProcess = totalObjectsToProcess + len(meshArray)
			
		print('Processing ' + str(totalObjectsToProcess) + ' static meshes...')
		
		if len(dict_meshes.items()) > 0:
			for materialName, meshArray in dict_meshes.items():
				if '_Mat' in materialName:
					materialName = materialName[:materialName.find('_Mat'):1]
	
				meshNumber = 0
				
				for meshObject in meshArray:
					childLocation = getObjectLocationFull(meshObject)
					childRotation = getObjectRotationFull(meshObject)
					childScale = getObjectScaleFull(meshObject)

					meshNumber = meshNumber + 1

					meshObject.name = sectorName + '_' + materialName + '_' + str(meshNumber)
					meshObject.data.name = meshObject.name
					meshObject.parent = sector

					meshObject.location = childLocation
					meshObject.rotation_euler = childRotation
					meshObject.scale = childScale
